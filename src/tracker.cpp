...
const cv::Mat &Tracker::ToGray(const cv::Mat &src, cv::Mat &dst) const {
  // The python driver reads frames with `cv::imread(path)`, whose default flag
  // is IMREAD_COLOR, so every image reaching the tracker is 3-channel BGR even
  // for a monochrome dataset. Nothing complained because the two consumers
  // tolerate it: `FastFeatureDetector::detect` calls cvtColor itself (once per
  // frame, thrown away afterwards) and `calcOpticalFlowPyrLK` happily tracks a
  // 3-channel patch. But `equalizeHist` and `cornerSubPix` both require a single
  // channel, and tracking on BGR is not the same problem as tracking on
  // luminance. Converting once here fixes both.
