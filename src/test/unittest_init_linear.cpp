...
//
// Everything here is built on one deliberate choice: the synthetic motion has a
// *closed form*, so the tests compare against an exact answer rather than
// against a finer-grained run of the same code. Constant body rate plus constant
// world-frame acceleration gives
//
//     R(t)     = exp(omega t)
//     p(t)     = v0 t + 0.5 a_w t^2
//     alpha(T) = 0.5 (a_w - g_w) T^2        beta(T) = (a_w - g_w) T
//
// with alpha and beta independent of how the rig is spinning -- so a
// preintegrator that agrees with those to 1e-12 while the rig turns at 1 rad/s
// is right for a reason, not right by construction.
#include <algorithm>
#include <cmath>
#include <limits>
#include <random>
...
InitProblem MakeProblem(const Truth &tr, const std::vector<Vec3> &pts,
  ...
  for (int k = 0; k < nframes; ++k) {
    ...
    fr.t = t;
    if (exact_pre) {
      // The preintegrals a perfect integrator with a perfect bias prior would
      // produce. Lets a Stage A test isolate the solver from the O(dt^2)
      // discretization error of the preintegrator, which has its own tests
      // above; `bg_prior` and `ba_prior` are ignored in this mode.
